...
from src.exercises.ohp import OverheadPress

VIDEO_CASES = [
    ("assets/videos/ohp_perfect.mp4", 5, "Strict form overhead press"),
    ("assets/videos/ohp_arching.mp4", 0, "Excessive back arch - should not count"),
    ("assets/videos/ohp_partial.mp4", 0, "Half-reps - should not count")
]
...
def analyze_video(video_path):
    ...
    while cap.isOpened():
        ...
        if landmarks:
            ohp.update(landmarks)
    cap.release()
    return ohp.reps_count


@pytest.mark.parametrize("video_file, expected_reps, description", VIDEO_CASES)
def test_ohp_scenarios(video_file, expected_reps, description):
